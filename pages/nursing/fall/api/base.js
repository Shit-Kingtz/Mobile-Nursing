...
// 详情ajax
let getAssessmentInfo = (id) => uniRequest.get(kfsys.prefixUrl + "api/v1/Hospitalized/NursingAssessment/AssesMonitorFall?AssessID=" + id)
...
let saveAssessment = (params, type, callback) => uni.request({
	url: kfsys.prefixUrl + 'api/v1/Hospitalized/NursingAssessment/AssesMonitorFall',
	method: type,
...
let getFinalModel = (params, model) => {
	if (params.AssessScore > 45) {
		model.MonitorFall = 2
	} else if (params.AssessScore <= 24) {
		model.MonitorFall = 0
	} else {
		model.MonitorFall = 1
	}
		...
		return new Promise((resolve, reject) => {
			saveBasicInfoModel(model, ajaxType, (result) => {
				...
				if (result.ResultType !== 0) {
					...
					if (!haveSupervisor) {
						goToSupervisor = params.SpeciallyMonitor || params.AssessScore > 45
					}
